...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!resendApiKey) {
      console.warn("[VERCEL] RESEND_API_KEY environment variable is missing.");
      return res.status(200).json({
        success: true,
        status: "saved_locally_only",
        message: "Note: Booking was saved locally, but no booking email was sent as RESEND_API_KEY is not defined in your Vercel Environment Variables."
      });
    }
    ...
    if (adminEmailResponse && adminEmailResponse.error) {
      console.error("[VERCEL] Resend returned an explicit API error:", adminEmailResponse.error);
    ...
    return res.status(500).json({ error: error.message || "Resend function failed." });
  }
}
